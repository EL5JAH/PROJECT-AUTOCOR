...
#
# Purpose:
# - Pull the Cisco pyATS Docker image during bootstrap
# - Create a writable pyATS workspace for container runtime state
# - Create a writable artifact directory for pyATS run outputs
# - Install a run-pyats wrapper for repeatable validation runs
#
# Important:
# - /opt/labrepo/pyats is mounted into the container as /pyats
# - /opt/labrepo/artifacts/pyats-runs/<timestamp> is mounted as /artifacts
...
PYATS_WORKSPACE="/opt/labrepo/pyats"
PYATS_RUNS_DIR="/opt/labrepo/artifacts/pyats-runs"

mkdir -p "${PYATS_WORKSPACE}"
mkdir -p "${PYATS_RUNS_DIR}"

chown -R cisco:cisco "${PYATS_WORKSPACE}"
chown -R cisco:cisco "${PYATS_RUNS_DIR}"

chmod -R 755 "${PYATS_WORKSPACE}"
chmod -R 755 "${PYATS_RUNS_DIR}"

echo "Pulling pyATS Docker image: ${PYATS_IMAGE}"
docker pull "${PYATS_IMAGE}"

cat > /usr/local/bin/run-pyats <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
# - Store run artifacts in timestamped directories
...
chmod 755 /usr/local/bin/run-pyats
...
###############################################################
# Lab environment helper aliases
###############################################################

alias lab='cd /opt/labrepo'

alias run-ansible='/opt/labrepo/scripts/run_ansible.sh'

alias run-pyats='/opt/labrepo/scripts/run_pyats.sh'

alias pyats-shell='docker run --rm -it -v /opt/labrepo:/workspace -w /workspace ciscotestautomation/pyats:latest bash'
EOF
...
cat >/etc/motd <<'EOF'
###############################################################
# PROJECT AUTOCOR - Portable Automation Lab
#
# Enterprise Network Automation & Validation Platform
###############################################################
...
---------------------------------------------------------------
  bootstrap-status       View bootstrap logs and status
  docker version         Verify Docker daemon connectivity
  docker ps              Verify Docker access without sudo
  ansible --version      Verify Ansible environment
  run-ansible            Execute Ansible automation wrapper
  run-pyats              Execute pyATS validation workflow

pyATS Operations
---------------------------------------------------------------
  pyats-shell            Open interactive pyATS container
  docker images          Verify pyATS container image

Optional Local GitLab
---------------------------------------------------------------
  validate-gitlab-lab    Validate local GitLab deployment
  docker logs gitlab     View GitLab container startup logs

Artifacts & Logs
---------------------------------------------------------------
  Ansible Runs:
    /opt/labrepo/artifacts/ansible-runs/
...
---------------------------------------------------------------
  - Docker
  - GitLab (optional local deployment)
  - Ansible
  - pyATS
...
---------------------------------------------------------------
  ...
  - pyATS executes inside containerized runtime
  - Validation artifacts persist under artifacts/
