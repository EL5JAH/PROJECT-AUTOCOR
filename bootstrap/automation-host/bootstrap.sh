...
#
# Purpose:
# - Pre-pull Cisco pyATS container image
# - Prepare pyATS runtime directories
# - Enable portable network validation workflows
# - Support CI/CD-ready pyATS execution
#
# Notes:
# - pyATS runs fully containerized
...
mkdir -p /opt/labrepo/artifacts/pyats-runs
mkdir -p /opt/labrepo/pyats/testbeds
mkdir -p /opt/labrepo/pyats/jobs
mkdir -p /opt/labrepo/pyats/tests

echo "Pulling pyATS Docker image..."
docker pull "${PYATS_IMAGE}"

echo "Validating pyATS container image..."

if ! docker image inspect "${PYATS_IMAGE}" >/dev/null 2>&1; then
    echo "Bootstrap failed: pyATS image missing"
    echo "BOOTSTRAP_STATE=failed_pyats_install" > "$STATUS_FILE"
    exit 1
fi
